...
main() {
  ...
  # —— 7. 自动验证品牌层在新基线上的可重放性（临时 worktree，不碰当前工作区）——
  # 失败不回滚基线：基线切换本身是成功且干净的，失败信号要留给维护者修 brand/；
  # 以非零退出阻断「升级完直接发布」的误操作。
  log "验证补丁/叠加层在新基线 $target_tag 上的可重放性…"
  if ! "$SCRIPT_DIR/apply-patches.sh" --check; then
    err "品牌层在新基线 $target_tag 上重放失败：基线文件已更新、gitlink 已暂存，"
    err "但必须先修复 brand/（诊断：scripts/apply-patches.sh --check-3way），"
    err "重跑 scripts/apply-patches.sh --check 通过后再提交/发布。"
    exit 1
  fi
  log "品牌层在新基线上重放通过。"
