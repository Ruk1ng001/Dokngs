...
#   （构建验证：cd infinite-canvas/web && bun install && bun run build，见 open-code-release.yml 的 upgrade-canvas job）
...
main() {
  ...
  # —— 7. 立即验证画布补丁在新基线上可 strict 重放（与 CI/发布同判据，临时 worktree、零副作用）——
  # 不通过时**不回滚基线**：以非 0 退出给出明确信号，避免「升级后要等 CI 才发现补丁冲突」。
  log "验证画布补丁在新基线上的 strict 重放（apply-canvas-patches.sh --check）…"
  if "$SCRIPT_DIR/apply-canvas-patches.sh" --check; then
    log "画布补丁在 ${target_tag} 上 strict 重放零冲突。"
    log "确认无误后提交：git commit brand/CANVAS_BASE_TAG brand/CANVAS_BASE_SHA infinite-canvas"
  else
    err "画布补丁在新基线 ${target_tag} 上无法干净重放（基线已更新、未回滚）。请先 rebase 画布补丁："
    err "  scripts/rebase-patches.sh --target canvas plan ${target_tag}"
    exit 1
  fi
}
