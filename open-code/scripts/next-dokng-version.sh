...
#
# 已有版本来源（按优先级）：
#   1. 环境变量 EXISTING_TAGS（空白/换行分隔）—— CI 用 `git tag` 填充（本仓不发 Release，tag 是唯一版本台账）；
...
#     供 GitHub Actions 用 steps.<id>.outputs.<key> 消费（open-code-release.yml 依赖此路径）。
#
# 历史坑（勿回退）：早期 stdout 也打四行 KEY=VALUE，导致 open-code-release.yml 的 upgrade-opencode job 里
